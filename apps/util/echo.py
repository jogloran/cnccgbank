...
def format_arg_value(arg_val, repr_function):
...
def echo(fn, repr_function=repr, write=sys.stderr.write):
    ...
    def format_arg_value_with_repr_function(v):
        return format_arg_value(v, repr_function)

    @functools.wraps(fn)
    def wrapped(*v, **k):
        # Collect function arguments by chaining together positional,
        # defaulted, extra positional and keyword arguments.
        positional = map(format_arg_value_with_repr_function, zip(argnames, v))
        defaulted = [format_arg_value_with_repr_function((a, argdefs[a]))
                     for a in argnames[len(v):] if a not in k]
        nameless = map(repr_function, v[argcount:])
        keyword = map(format_arg_value_with_repr_function, k.items())
